...
var presets = map[project.Mode]ModePreset{
	...
	project.ModeBugBounty: {
		...
		RecommendedTools: map[Phase][]string{
			...
			PhaseAssetDiscovery: {"crtsh", "dnsx", "historical_urls"},
			PhaseValidation:     {"httpx_probe", "gowitness", "msf_smb_version", "msf_ssh_version", "msf_rdp_scanner"},
			PhaseExposure:       {"nuclei", "katana", "testssl", "msf_smb_enumshares", "msf_ftp_anonymous", "msf_snmp_login", "msf_vnc_none_auth"},
			PhaseEnrichment:     {"trufflehog", "shodan"},
		},
